          ...
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form class="space-y-4 md:space-y-6" @submit.prevent="checkUserPassword">
              ...
              <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                ...
                <a @click="dialog = true" class="font-medium text-primary-600 hover:underline dark:text-primary-500"
                  >Regístrate</a
                >
              </p>
            </form>
...
const checkUserPassword = async () => {
    ...
    try {
        const res = await axios.get(
            `http://localhost:3000/usuarios?username=${userName.value}`
        );

        if (res.data[0].contraseña === password.value) {
            storeUser.loginUser(res.data[0].username)
            router.push('/Lobby')
        } else {
