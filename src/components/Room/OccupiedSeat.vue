...
<template>
  <div>
    <CardsHand :index="index" :hand="seat.hand" :user="seat.user" :handCards="handCards" :seats="seats" :room="room"
      class="flex justify-center" />
    <div class="w-52 h-28 Seat rounded-full border-2 border-amber-400 flex flex-row justify-between">
      <div class="flex w-10">
        <div class="w-52">
          <button v-if="seat.user === storeUser.user.displayName" type="button" @click="leaveSeat()"
      ...
      <div class="flex flex-col w-40 ml-6 mt-3">
        <div class="flex flex-row justify-around w-36 mr-2">
          <img v-if="seat.photoUser" class="w-10 h-10 rounded-full" :src="seat.photoUser" alt="User Photo" />
          <img v-else class="w-10 h-10 rounded-full mt-3"
            src="../../assets/images/poker-king-beard-logo-design-260nw-2168601229.webp" alt="User Photo" />
        ...
        <div v-if="index === 2" class="flex flex-row justify-start w-52 mt-5 -ml-20 overflow-visible">
          <div v-if="seat.potPlayer !== 0" class="flex flex-col justify-center -ml-10 overflow-visible">
            <img src="../../assets/images/newnewstack-removebg-preview.png" class="w-12 h-12 rounded-xl" alt="" />
            <p class="text-white font-bold text-center">{{ seat.potPlayer }}</p>
          </div>
          <div v-if="seat.dealer !== '' && seat.dealer !== 'dealer'" class="ml-16 w-12 h-12 text-center rounded-full">
            <img v-if="seat.dealer === 'bb'" src="../../assets/images/BB-removebg-preview.png" alt="" />
            <img v-if="seat.dealer === 'sb'" src="../../assets/images/SB-removebg-preview.png" alt="" />
          </div>
        ...
        <div v-if="index === 1" class="flex flex-row justify-start w-52 mt-5 overflow-visible">
          ...
          <div v-if="seat.dealer !== '' && seat.dealer !== 'dealer'" class="ml-16 w-12 h-12 rounded-full">
            <img v-if="seat.dealer === 'bb'" src="../../assets/images/BB-removebg-preview.png" alt="" />
            <img v-if="seat.dealer === 'sb'" src="../../assets/images/SB-removebg-preview.png" alt="" />
          </div>
          <div v-if="seat.dealer !== '' && seat.dealer === 'dealer'" class="w-10 h-10 ml-5">
