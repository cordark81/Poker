<template>
	<div
		class="absolute bottom-0 left-0 bg-slate-800 w-3/5 h-56 overflow-auto"
		ref="scrollContainer"
	>
		<form
			class="mb-4 border-t border-gray-300 sticky top-0"
			@submit.prevent="sendMessage()"
		>
			<div class="flex">
				<input
					v-model="text"
					class="flex-1 border rounded-lg rounded-tl-none rounded-tr-none py-2 px-4 outline-none"
					type="text"
		...
		<div>
			<div v-for="message in messages" class="flex" :key="message.id">
				<div class="flex items-center mb-2">
					<img
						v-if="message.photoUser"
						class="w-10 h-10 rounded-full mr-2 ml-5 border-2 border-amber-400"
