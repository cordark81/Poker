...
<template>
	<div class="absolute bottom-0 right-0 flex flex-col justify-center items-center w-64 h-64 overflow-visible">
		<div class="flex flex-col mb-48 mt-2 mr-48">
			...
			<ButtonConsole v-if="storeGame.verifySimilarPots(seats) === true"
				@click="storeConsole.checkConsole(seats, room)" :text="'Check'" :color="classGrayColor" class="mt-2" />
			<ButtonConsole v-if="storeGame.verifySimilarPots(seats) === false" @click="logicCall()" :text="'Call'"
				class="mt-2" :color="classGrayColor" />
