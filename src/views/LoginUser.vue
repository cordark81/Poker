<template>
  <div class="background-login flex flex-col items-center pt-72 h-screen">
    ...
    <div class="notification mt-4" v-show="errorMessage">
    ...
    >
      <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
        ...
        <form class="space-y-4 md:space-y-6" @submit.prevent="doLogin">
          <div>
            <label for="email" class="text-white block mb-2 text-sm font-medium dark:text-white">Correo electrónico</label>
            <input
              v-model="email"
              type="email"
              name="email"
  ...
  <div v-show="showForgotPasswordModal" class="background-forgot-password fixed top-0 left-0 flex items-center justify-center w-screen h-screen z-50">
    <div class="w-5/6 sm:w-4/5 md:w-3/4 lg:w-1/2 xl:w-1/3 bg-black rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 p-6 border-2 border-amber-400">
      ...
      <p class="text-white dark:text-gray-300">Ingresa tu correo electrónico para recibir instrucciones sobre cómo restablecer tu contraseña. El correo puede llegar a la carpeta de spam, así que asegúrate de revisarla también.</p>
...
const getMessageError = (errorCode) => {
  switch (errorCode) {
    ...
    default:
      return 'Se produjo un error en la autenticación.';
  }
};
...
.notification {
  max-width: 30rem;
  background-color: rgba(255, 0, 0, 0.9); /* Cambia el color de fondo según tus preferencias */
  color: white;
